...
import { gameWorker } from "../worker/gameWorker.js";
...
async function runBattleFlowTest() {
  ...
  try {
    ...
    let participantIds = [];
    if (startPayload.participants && Array.isArray(startPayload.participants)) {
    ...
    for (let i = 0; i < submissionResults.length; i++) {
      ...
      const problemIdVal =
        startPayload.problem && startPayload.problem.id
          ? startPayload.problem.id
          : "prob_101";
      const problemsJson = JSON.stringify([{ id: problemIdVal }]);
    ...
    const gameResultData = {
      matchId: startPayload.matchId || "battle-" + roomId,
      roomId: roomId,
      winnerId: winnerId,
