...
export function parseCreateRoomRequest(body = {}) {
  const roomTitle = typeof body.roomTitle === 'string'
    ? body.roomTitle.trim()
    : '';
  const playerMode = typeof body.playerMode === 'string'
    ? body.playerMode.trim()
    : '';
  const gameMode = typeof body.gameMode === 'string'
    ? body.gameMode.trim()
    : '';
  ...
  if (!GAME_MODES.has(gameMode)) {
    throw invalidRoomRequest('게임 모드는 normal 또는 item만 선택할 수 있습니다.');
  }

  if (!DIFFICULTIES.has(difficulty)) {
    throw invalidRoomRequest('난이도는 쉬움, 보통, 어려움 중에서 선택해 주세요.');
  }
  ...
  if (!Number.isInteger(problemCount) || problemCount < 3 || problemCount > 10) {
